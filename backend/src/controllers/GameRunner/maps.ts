...
export class GameMap {
    ...
    constructor(private x: number, private y: number, private player: number) {
        ...
        while (this.blocks.length <= ((x * y * n) + player)) {
            var randX = this.random(0, x)
            var randY = this.random(0, y)
            var check = _.findIndex(this.blocks, (res) => {
            ...
            if (check == -1) {
                ...
                let b = new Block(type, randX, randY)
                this.blocks.push(b)
    ...
    private walker(player:Block,dest:number){
        var destX=player.x,destY=player.y;
        ...
        let check = _.findIndex(this.blocks, (res) => {
